...
2. Run the bootstrap script (installs dependencies, runs migrations, builds assets):
```bash
docker compose exec -u application laravel bash /app/.docker/bootstrap.sh
```
...
```bash
docker compose exec -u application laravel bash
```
...
Watch frontend assets for changes (inside container):
```bash
npm run watch
```
